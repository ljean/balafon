# -*- coding: utf-8 -*-
"""
This script detects the modified files thanks to 'hg status' command
...
class Hg(CommandWrapper):
    """Wrapper to hg commands sent thanks to subprocess"""
    ...
    def get_root(self):
        ...
        output, error_code = self._run_command('hg root')
        if error_code:
            raise Exception("Hg.get_root failed: Error {0}".format(error_code))
        return output.strip()
    ...
    def get_all_files(self):
        ...
        """
        output, error_code = self._run_command('hg manifest --all')
        if error_code:
            raise Exception("Hg.get_all_files failed: Error {0}".format(error_code))
    ...
    def get_changes(self):
        ...
        returns a list of modified files by running a git status command
        """
        output, error_code = self._run_command('hg status')
        if error_code:
            raise Exception("Hg.get_changes failed: Error {0}".format(error_code))
...
def main():
    ...
    if pylint.is_ready():
        mercurial = Hg()
        if analyze_all:
            files = mercurial.get_all_files()
        else:
            files = mercurial.get_changes()
