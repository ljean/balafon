...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for e in orm.Entity.objects.all():
            group, _is_new = orm.Group.objects.get_or_create(name=e.relationship.name)
            group.entities.add(e)
            group.save()
            
            group, _is_new = orm.Group.objects.get_or_create(name=e.type.name)
            group.entities.add(e)
            group.save()
            
            if e.activity_sector:
                group, _is_new = orm.Group.objects.get_or_create(name=e.activity_sector.name)
                group.entities.add(e)
                group.save()
