...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding M2M table for field contacts on 'Action'
        m2m_table_name = db.shorten_name(u'Crm_action_contacts')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('action', models.ForeignKey(orm[u'Crm.action'], null=False)),
            ('contact', models.ForeignKey(orm[u'Crm.contact'], null=False))
        ))
        db.create_unique(m2m_table_name, ['action_id', 'contact_id'])

        # Adding M2M table for field entities on 'Action'
        m2m_table_name = db.shorten_name(u'Crm_action_entities')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('action', models.ForeignKey(orm[u'Crm.action'], null=False)),
            ('entity', models.ForeignKey(orm[u'Crm.entity'], null=False))
        ))
        db.create_unique(m2m_table_name, ['action_id', 'entity_id'])


    def backwards(self, orm):
        # Removing M2M table for field contacts on 'Action'
        db.delete_table(db.shorten_name(u'Crm_action_contacts'))

        # Removing M2M table for field entities on 'Action'
        db.delete_table(db.shorten_name(u'Crm_action_entities'))
