...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        newsletter, _is_new = orm.SubscriptionType.objects.get_or_create(name="Newsletter")
        third_party, _is_new = orm.SubscriptionType.objects.get_or_create(name="Partenaires")
        
        for contact in orm.Contact.objects.all():
            for (attr, s_t) in (('accept_3rdparty', third_party), ('accept_newsletter', newsletter)):
                if getattr(contact, attr):
                    s, is_new = orm.Subscription.objects.get_or_create(subscription_type=s_t, contact=contact)
                    if is_new:
                        s.accept_subscription=True
                        s.subscription_date=datetime.datetime.now()
                        s.save()
